	  ...
	  bool tankControl = true;		// if true tankcontrol if false arcade control
		...
		if(tankControl) {
            ...
            if(abs(left) <= deadZone) { left = 0; }
				 ...
				 motorSet(LM_F_DRIVE, left); // set left wheels
				 motorSet(RM_F_DRIVE, -right); // set right wheels -- mirrored motor in chassis
				 delay(20);
			 }
		} else {
			int power;
        ...
        motorSet(LM_F_DRIVE, power + turn); // set left wheels
        motorSet(RM_F_DRIVE, power - turn); // set right wheels
        delay(20);
	  }
	}
}
